...
_TD.a.push(function (TD) {
	...
	var map_obj = {
		_init: function (cfg) {
			cfg = cfg || {};
			this.grid_x = cfg.grid_x || 10;//16
			this.grid_y = cfg.grid_y || 10;//16
			this.x = cfg.x || 0;//zuo shang jiao 
			this.y = cfg.y || 0;//zuo shangjiao
			...
			var _this = this;
			if (cfg.grids_cfg) {
				TD.lang.each(cfg.grids_cfg, function (obj) {//pos 是这个元素的位置坐标数组
		...
		//预建设  是指用鼠标拖动东西到画布的上面
		preMan : function(building_type) {
			TD.mode = "build";
			if (this.pre_building) {
				this.pre_building.remove();
			}
			this.pre_building = new TD.Man(this.id + "-" + "pre-man-"
							+ TD.lang.rndStr(), {
						type : building_type,
						map : this,
						is_pre_building : true
					});
			this.scene.addElement(this.pre_building, 1, 4);
		},
	...
	TD.Map = function (id, cfg) {
		...
		TD.lang.mix(map, map_obj);//map 是接口 实现类是map_obj
		map._init(cfg);
	...
	var map_selecthl_obj = {
		_init: function (cfg) {//每个地图都初始化一个高亮框
			this.map = cfg.map;
	 ...
	 *		 }
	 */
	TD.MapSelectHighLight = function (id, cfg) {//main-map-hl  panel-map-hl
	...
	var mmm_obj = {
		...
		render: function () {
			...
			//ctx.drawImage(TD.background_img,0,0);
