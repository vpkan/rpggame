...
_TD.a.push(function (TD) {
	...
	var building_obj = {
		 ...
		 */
		locate: function (grid) {//相当于build的初始化了
			this.grid = grid;
			this.map = grid.map;
...
/**
		...
		beHit: function (monster, damage) {
			...
			if(monster.addtional){
				monster.addtional(this);
...
//			building.killed ++;
		...
		render: function () {//TD.log("building.render");
			...
			if ((this.type == "laser_gun")  && this.target && this.target.is_valid) {
				// 画激光
				ctx.setLineWidth ( 3);
				ctx.setStrokeStyle ( "rgba(50, 50, 200, 0.5)");
				ctx.beginPath();
				...
				ctx.stroke();
				ctx.setLineWidth ( 1);
				ctx.setStrokeStyle("rgba(150, 150, 255, 0.5)");
				ctx.beginPath();
		...
		onEnter: function () {//所有的元素的onenter事件都是他本身定义的 那为什么我定义的man接收不到呢
