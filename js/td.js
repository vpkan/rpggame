/**
 *
*
* Author:
*	zzw <zzw1986@gmail.com>
*	
*
* File: td.js
* Create Date: 2013-9-23 8:29
*
*
*/
var _TD = {
	a: [],
	init: function (td_board, is_debug) {//_TD.init("td-board", true);
		delete this.init; // 一旦初始化运行，即删除这个入口引用，防止初始化方法被再次调用
		//首先笤俑_TD.init方法  方法直接创造了一下对象
		var i, TD = {//预先定义了 i 和td对象（一个hashmap）
			version: "0.1.16", // 版本命名规范参考：http://semver.org/
			is_debug: !!is_debug,
			angle:0,//倾斜的角度
			tilt_angle:60,
			radian:0,//折算成pi值
			tilt_radian:0,//折算成pi值
			zoom:1,
			is_paused: true,//一开始是停止的
			width: 16, // 横向多少个格子
			height: 16, // 纵向多少个格子
			show_monster_life: true, // 是否显示怪物的生命值
			fps: 0,
			exp_fps: 24, // 期望的 fps
			exp_fps_half: 12,
			exp_fps_quarter: 6,
			exp_fps_eighth: 4,
			stage_data: {},
			defaultSettings: function () {
				return {
					step_time: 36, // 每一次 step 循环之间相隔多少毫秒
					grid_size: 32, // px
					padding: 10, // px 和容器的边框距离
					global_speed: 0.1 // 全局速度系数
				};
			},

			/**
			 * 初始化
			 * @param ob_board
			 */
			init: function (ob_board/*, ob_info*/) {//console.log("init 方法初始化");
				//console.log(this.eventManager);
				this.obj_board = TD.lang.$e(ob_board);//"td-board" canvas 的父亲div
				this.canvas = this.obj_board.getElementsByTagName("canvas")[0];//容易理解
				//this.obj_info = TD.lang.$e(ob_info);
				if (!this.canvas.getContext) return; // 不支持 canvas
				this.ctx = new TD.ctx_fake(this.canvas);//得到2d场景
				//this.ctx = this.canvas.getContext("2d");//得到2d场景
			  	this.monster_type_count = TD.getDefaultMonsterAttributes(); // 一共有多少种怪物
				this.iframe = 0; // 当前播放到第几帧了
				this.last_iframe_time = (new Date()).getTime();
				this.fps = 0;
				/*
				this.img=new Image();
				this.img.src="img/grass.jpg";
				this.background_img=new Image();
				this.background_img.src="img/8.jpg";*/
				
				//test
				this.start();
			
			
			},

			/**
			 * 开始游戏，或重新开始游戏
			 */
			start: function () {
				/*
				迷雾	this.image_box=new Image();
				this.image_box .src="img/frog.png";
				*/
				/*=new Array[16];
				this.ctx.drawImage("image/grass.png",0,0);
				for(var i=0;i<16;i++){
					var index_x ,index_y;
					index_x = parseInt(i/4);
					index_y= i%4;
					var x  =64*i;
					y=64*i;
					this.image_box[i]=
				}*/
			//window.console.log("停止setTimeout this._st");
				clearTimeout(this._st);//停止重复
				//console.log("Start!");//日志记录
				var _this = this;
				this._exp_fps_0 = this.exp_fps - 0.4; // 下限 23.6
				this._exp_fps_1 = this.exp_fps + 0.4; // 上限23.4

				this.mode = "normal"; // mode 分为 normail（普通模式）及 build（建造模式）两种
				this.eventManager.clear(); // 清除事件管理器中监听的事件 转td-event.js
				this.lang.mix(this, this.defaultSettings());
				
				//一些需要运算过的数据 以方便以后重复使用
				this.width_px_half=this.grid_size/2* this.width;
				this.height_px_half= this.grid_size/2* this.height;
				this.radian=GetRadian(this.angle);
				this.tilt_radian=GetRadian(this.tilt_angle);
				//console.log("在td.js的start方法中创建stage对象");//日志记录
				this.stage = new TD.Stage("stage-main", TD.getDefaultStageData("stage_main"));

				this.canvas.setAttribute("width", this.stage.width);
				this.canvas.setAttribute("height", this.stage.height);

				this.canvas.onmousemove = function (e) {//注册鼠标移动事件
					var xy = _this.getEventXY.call(_this, e);
					_this.hover(xy[0], xy[1]);
				};
				this.canvas.onclick = function (e) {//注册鼠标点击事件
					var xy = _this.getEventXY.call(_this, e);
					_this.click(xy[0], xy[1]);
				};
				this.canvas.onmousedown = function (e) {//注册鼠标点击事件
					if(e.button==2){
						e.cancelBubble=true;
						var xy = _this.getEventXY.call(_this, e);
					_this.rclick(xy[0], xy[1]);
					}
				};
				
				
				this.is_paused = false;//在这之前都是画布的成型
				//TD.log("stage start");
				this.stage.start();
				//让所有的东西全部倾斜角度
				/*if(TD.angle>0){
					this.ctx.translate(TD.grid_size*TD.width/2,TD.grid_size*TD.height/2);
					this.ctx.rotate(Math.PI*(90-TD.angle)/180);
					this.ctx.scale(TD.zoom, TD.zoom); //x坐标不缩放，y坐标变成2倍
					this.ctx.translate(-TD.grid_size*TD.width/2,-TD.grid_size*TD.height/2);
				}*/
				//TD.log("stage start 完毕");
				//grid
				//TD.log(TD.stage.current_act.current_scene._render_elements[2]);return;
				//第3个元素里有256个grid元素
				this.step();

				return this;
			},

			/**
			 * 作弊方法
			 * @param cheat_code
			 *
			 * 用例：
			 * 1、增加 100 万金钱：javascript:_TD.cheat="money+";void(0);
			 * 2、难度增倍：javascript:_TD.cheat="difficulty+";void(0);
			 * 3、难度减半：javascript:_TD.cheat="difficulty-";void(0);
			 * 4、生命值恢复：javascript:_TD.cheat="life+";void(0);
			 * 5、生命值降为最低：javascript:_TD.cheat="life-";void(0);
			 */
			checkCheat: function (cheat_code) {
				switch (cheat_code) {
					case "money+":
						this.money += 1000000;
						this.log("cheat success!");
						break;
					case "life+":
						this.life = 100;
						this.log("cheat success!");
						break;
					case "life-":
						this.life = 1;
						this.log("cheat success!");
						break;
					case "difficulty+":
						this.difficulty *= 2;
						this.log("cheat success! difficulty = " + this.difficulty);
						break;
					case "difficulty-":
						this.difficulty /= 2;
						this.log("cheat success! difficulty = " + this.difficulty);
						break;
				}
			},

			/**
			 * 主循环方法
			 */
			step: function () {
				
				if (this.is_debug && _TD && _TD.cheat) {
					// 检查作弊代码
					this.checkCheat(_TD.cheat);
					_TD.cheat = "";
				}

				if (this.is_paused) return;

				this.iframe ++; // 当前总第多少帧
				if (this.iframe % 50 == 0) {
					// 计算 fps
					var t = (new Date()).getTime(),
						step_time = this.step_time;
					this.fps = Math.round(500000 / (t - this.last_iframe_time)) / 10;
					this.last_iframe_time = t;

					// 动态调整 step_time ，保证 fps 恒定为 24 左右
					if (this.fps < this._exp_fps_0 && step_time > 1) {
						step_time --;
					} else if (this.fps > this._exp_fps_1) {
						step_time ++;
					}
//					if (step_time != this.step_time)
//						TD.log("FPS: " + this.fps + ", Step Time: " + step_time);
					this.step_time = step_time;
				}
				if (this.iframe % 2400 == 0) TD.gc(); // 每隔一段时间自动回收垃圾
				//TD.log("stage step");
				this.stage.step();
				//TD.log("stage render");
				this.stage.render();
				//
				var _this = this;
				//TD.log("这里的代码暂时注释");
				this._st = setTimeout(function () {
					_this.step();
					//console.log("_this.step()主要的循环方法");
				}, this.step_time);
			},

			/**
			 * 取得事件相对于 canvas 左上角的坐标
			 * @param e
			 */
			getEventXY: function (e) {
				var wra = TD.lang.$e("wrapper"),
					x = e.clientX - wra.offsetLeft - this.canvas.offsetLeft + Math.max(document.documentElement.scrollLeft, document.body.scrollLeft),
					y = e.clientY - wra.offsetTop - this.canvas.offsetTop + Math.max(document.documentElement.scrollTop, document.body.scrollTop);

				return [x, y];
			},

			/**
			 * 鼠标移到指定位置事件
			 * @param x
			 * @param y
			 */
			hover: function (x, y) {
				this.eventManager.hover(x, y);
			},

			/**
			 * 点击事件
			 * @param x
			 * @param y
			 */
			click: function (x, y) {
				this.eventManager.click(x, y);
			},

			
			/**
			 * 点击事件
			 * @param x
			 * @param y
			 */
			rclick: function (x, y) {
				this.eventManager.rclick(x, y);
			},

			/**
			 * 是否将 canvas 中的鼠标指针变为手的形状
			 * @param v {Boolean}
			 */
			mouseHand: function (v) {
				this.canvas.style.cursor = v ? "pointer" : "default";
			},

			/**
			 * 显示调试信息，只在 is_debug 为 true 的情况下有效
			 * @param txt
			 */
			log: function (txt) {
				this.is_debug && window.console && console.log && console.log(txt);
			},

			/**
			 * 回收内存
			 * 注意：CollectGarbage 只在 IE 下有效
			 */
			gc: function () {
				if (window.CollectGarbage) {
					CollectGarbage();
					setTimeout(CollectGarbage, 1);
				}
			}
		};

		for (i = 0; this.a[i]; i ++) {
			// 依次执行添加到列表中的函数 注册td的各个小部件
			this.a[i](TD);
		}
		
		delete this.a;
		//调用TD.init
		TD.init(td_board);
	}
};